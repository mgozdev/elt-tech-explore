...
<xsl:for-each select="m1:invstOrSecs/m1:invstOrSec">
	 ...
	 <xsl:choose>
	 <xsl:when test="m1:notDissem = 'true'">
...
<br/>
		...
		<h4>Item D.6. Is the investment a Restricted Security?</h4>
		...
		<h4>Item D.8. Indicate the level within the fair value hierarchy in
			which the fair value measurements fall pursuant to U.S. Generally
			Accepted Accounting Principles (ASC 820, Fair Value Measurement).
			[1/2/3] Report “N/A” if the investment does not have a level
			associated
			with it (i.e., net asset value used as the practical expedient).</h4>
  ...
  <xsl:otherwise>
		...
		<h4>Item C.6. Is the investment a Restricted Security?</h4>
		<xsl:call-template name="c6" />
		...
		<h4>Item C.8. Indicate the level within the fair value hierarchy in which the fair value measurements fall pursuant to U.S. Generally Accepted Accounting Principles (ASC 820, Fair Value Measurement). [1/2/3]  Report “N/A” if the investment does not have a level associated with it (i.e., net asset value used as the practical expedient).</h4>
		<xsl:call-template name="c8" />
   ...
   <xsl:param name="path"/>
	...
	<p>Balance. Indicate whether amount is expressed in number of shares, principal amount, or other units.
	 For derivatives contracts, as applicable, provide the number of contracts.</p>
		...
		<tr>
			...
			<xsl:if test="$path/m1:isIncludedFunds80PercentBasket">
				<tr>
					<td class="label">If the Fund is required to adopt a policy as described in rule 35d-1(a)(2)(i) or (a)(3)(i) [17 CFR 270.35d-1(a)(2)(i) or (3)(i)], is the investment included in the Fund’s 80% basket, as defined in rule 35d-1(g), as applicable? [Y/N]
					</td>
					<td>
						<div class="fakeBox4">
							<xsl:value-of select="$path/m1:isIncludedFunds80PercentBasket" />
							<span>
								<xsl:text>&#160;</xsl:text>
							</span>
						</div>
   ...
   <table>
   	<tr>
			...
			<td>
				<div class="fakeBox">
					...
					<xsl:value-of select="$path/m1:assetConditional/@assetCat" />
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
				</div>
		...
		<xsl:if test="$path/m1:assetConditional/@assetCat">
   	<tr>
   	...
   	<tr>
			...
			<td>
				<div class="fakeBox">
					...
					<xsl:value-of select="$path/m1:issuerConditional/@issuerCat" />
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
				</div>
      ...
      <xsl:if test="$path/m1:issuerConditional/@issuerCat">
   	<tr>
			<td class="label">If "other", provide a brief description.
			</td>
			<td>
	...
	<xsl:template name="c7">
		...
		<p>a. Liquidity classification information. For portfolio investments of open-end management investment companies, provide the liquidity classification(s) for each portfolio 
		...
		percentage amount attributable to each classification.</p>
		<table>
		...
		<table>
		<xsl:choose>
		<xsl:when test="m1:fundCats/m1:fundCat">
			<xsl:for-each select="m1:fundCats/m1:fundCat" >
			<tr>
				...
				<td>
						<div class="fakeBox3">
							<xsl:call-template name="enumAbbr">
							<xsl:with-param name="abbr"		select="string(@category)" />
		...
		<p>b. If attributing multiple classification categories to the holding, indicate which of the three circumstances listed in the Instructions to Item C.7 is applicable. </p>
	    
	    <p><b>Instructions to Item C.7</b> 
	    ...
	    In (1) and (2), a fund would classify using the reasonably anticipated trade size for each portion of the position.</p>
	    <table>
		<xsl:choose>
		<xsl:when test="m1:fundCats/m1:circumstances">
		<xsl:for-each select="m1:fundCats/m1:circumstances" >
			<tr>
				<td class="label">Circumstances
				</td>
				<td>
				<div class="fakeBox3">
					<xsl:call-template name="enumAbbr">
								<xsl:with-param name="abbr"		select="." />
								<xsl:with-param name="category"	select="'LIQUIDITY_CIRCUMSTANCE_TYPE'" />
						</xsl:call-template>
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
	...
	<p>iii. Description of the reference instrument, including the name of issuer, title of issue, and currency in 
	 ...
	 (if CUSIP and ISIN are not available), or other identifier (if CUSIP, ISIN, and ticker are not available).<br/>
	  ...
	  <xsl:for-each select="$path/m1:dbtSecRefInstruments/m1:dbtSecRefInstrument">
		<table>
		<tr>
			<td>
					Reference Instrument Record:		<xsl:value-of select="position()"></xsl:value-of>
			</td>
		</tr>
		<tr>
...
<xsl:choose>
	...
	<tr>
		...
		<table>
		<xsl:for-each select="$derivative/m1:counterparties">
		<tr>
			<td>
					Counterparty Record:		<xsl:value-of select="position()"></xsl:value-of>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div class="fakeBox">
					<xsl:value-of select="m1:counterpartyName" />
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
				</div>
		...
		<tr>
			...
			<td>
				<div class="fakeBox2">
					<xsl:value-of select="m1:counterpartyLei" />
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
				</div>
	...
	<xsl:param name="derivative"/>
	<xsl:if test="$derivative/m1:derivAddlInfo">
	<br/>
	...
	<xsl:if test="$derivative/m1:fixedRecDesc or $derivative/m1:floatingRecDesc or $derivative/m1:recAmtCat  or $derivative/m1:otherRecDesc">
	...
	<table>
		<tr>
			...
			<td>
				<span class="yesNo">
					<xsl:choose>
						<xsl:when test="$derivative/m1:fixedRecDesc or $derivative/m1:floatingRecDesc or $derivative/m1:recAmtCat or $derivative/m1:otherRecDesc">
							<xsl:choose>
							...
							<xsl:choose>
								...
								<xsl:choose>
								<xsl:when test="string($derivative/m1:otherRecDesc)">
									<img
										src="Images/radio-unchecked.jpg"
										alt="Fixed is not checked" />
									Fixed
	 ...
	 <xsl:when test="$derivative/m1:otherRecDesc">
	 <tr>
			<td class="label">Description of Other Receipts
			</td>
			<td>
				<div class="fakeBox3">
					<xsl:value-of select="$derivative/m1:otherRecDesc" />
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
				</div>
...
<!--DESCRIPTION_TERM_TYPE START-->
		...
		<xsl:if test="$derivative/m1:fixedPmntDesc or $derivative/m1:floatingPmntDesc or $derivative/m1:pmntAmtCat or  $derivative/m1:otherPmntDesc">
		...
		<tr>
			...
			<td>
				<span class="yesNo">
					<xsl:choose>
						<xsl:when test="$derivative/m1:fixedPmntDesc or $derivative/m1:floatingPmntDesc or $derivative/m1:pmntAmtCat or  $derivative/m1:otherPmntDesc ">
							<xsl:choose>
							...
							<xsl:choose>
								<xsl:when test="$derivative/m1:otherPmntDesc">
									<img
										src="Images/radio-unchecked.jpg"
										alt="Fixed is not checked" />
									Fixed
		...
		<xsl:when test="$derivative/m1:otherPmntDesc">
		<tr>
			<td class="label">Description of Other Payments
			</td>
			<td>
				<div class="fakeBox3">
					<xsl:value-of select="$derivative/m1:otherPmntDesc" />
					<span>
						<xsl:text>&#160;</xsl:text>
					</span>
				</div>
...
<!--DESCRIPTION_TERM_TYPE END-->	
	 ...
	 <tr><td>iii.	Upfront payments or receipts</td></tr>
		...
		<tr>
			...
			<td>
				<div class="fakeBox4">
					<xsl:call-template name="enumAbbr">
							<xsl:with-param name="abbr"		select="$derivative/m1:pmntCurCd" />
							<xsl:with-param name="category"	select="'ISO_CURRENCY_CODE'" />
					</xsl:call-template>
					<span>
						<xsl:text>&#160;</xsl:text>
	 ...
	 <xsl:if test="$derivative/m1:upfrontRcpt">
		...
		<tr>
			...
			<td>
				<div class="fakeBox4">
					<xsl:call-template name="enumAbbr">
							<xsl:with-param name="abbr"		select="$derivative/m1:rcptCurCd" />
							<xsl:with-param name="category"	select="'ISO_CURRENCY_CODE'" />
					</xsl:call-template>
					<span>
						<xsl:text>&#160;</xsl:text>
   ...
   <table>
   	<xsl:if test="$derivative/m1:shareNo">
		...
		<tr>
			<td class="label">vi.	Exercise Price Currency Code
			</td>
			<td>
				<div class="fakeBox4">
					<xsl:call-template name="enumAbbr">
							<xsl:with-param name="abbr"		select="$derivative/m1:exercisePriceCurCd" />
							<xsl:with-param name="category"	select="'ISO_CURRENCY_CODE'" />
					</xsl:call-template>
					<span>
						<xsl:text>&#160;</xsl:text>
		...
		<tr>
			<td class="label">vii.	Expiration date.
			</td>
			<td>
				<div class="fakeBox4">
      ...
      <xsl:if test="$derivative/m1:unrealizedAppr">
		<tr>
			<td class="label">ix.	Unrealized appreciation or depreciation. Depreciation shall be reported as a negative number.
			</td>
			<td>
				<div class="fakeBox4">
					<xsl:value-of select="$derivative/m1:unrealizedAppr" />
